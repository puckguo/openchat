...
copy_project_files() {
    ...
    if [[ "$(basename "$SCRIPT_DIR")" == "deploy" ]]; then
        ...
        print_info "检测到脚本位于 deploy 目录，使用项目根目录: $PROJECT_ROOT"
    else
        # 脚本在项目根目录中
    ...
    if [ ! -f "$PROJECT_ROOT/package.json" ]; then
        print_error "未找到 package.json，请确保项目文件完整"
        exit 1
    fi

    # 复制文件（排除 deploy 目录和日志等）
    rsync -av --exclude='deploy' --exclude='.git' --exclude='node_modules' --exclude='*.log' --exclude='data' "$PROJECT_ROOT/" "$INSTALL_DIR/"
