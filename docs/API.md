# OpenCode Chat API Documentation

This document describes the WebSocket API for OpenCode Chat, including message formats, events, and integration examples.
...
ws.onopen = () => {
  console.log('Connected to OpenCode Chat');
};
```
...
class OpenCodeChat {
  ...
  setupEventHandlers() {
    ...
    this.ws.onclose = () => {
      console.log('Disconnected from OpenCode Chat');
    };
  }
